...
export function BookingSummary({ pickup, destination, route, values, price }: BookingSummaryProps) {
  return (
    <aside className="relative z-[500] m-4 rounded-[8px] border border-white/70 bg-white/88 p-5 shadow-[0_15px_45px_rgba(7,23,39,.18)] backdrop-blur-xl lg:absolute lg:right-5 lg:bottom-5 lg:left-5 lg:m-0">
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-[1.2fr_1.2fr_.6fr_.6fr_.8fr]">
        ...
        <div><p className="text-[10px] font-extrabold tracking-[1.3px] text-muted uppercase">Distance</p><p className="mt-1 text-xs font-bold text-navy">{route ? `${route.distanceKm.toFixed(1)} km` : '—'}</p></div>
        <div><p className="text-[10px] font-extrabold tracking-[1.3px] text-muted uppercase">Durée</p><p className="mt-1 text-xs font-bold text-navy">{route ? `${route.durationMinutes} min` : '—'}</p></div>
